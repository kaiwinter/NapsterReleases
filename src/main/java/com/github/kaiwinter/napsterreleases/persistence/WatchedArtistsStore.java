...
/**
 * Persists the watched artists with their latest album release. The FX Bean {@link WatchedArtist} is persisted directly therefore the
...
public class WatchedArtistsStore {
	...
	public Set<WatchedArtist> loadWatchedArtists() {
		...
		try {
			...
			Gson gson = new GsonBuilder().registerTypeAdapter(ObjectProperty.class, new LastReleaseDeserializer()).create();
